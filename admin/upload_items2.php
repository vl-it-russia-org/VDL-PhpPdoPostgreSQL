...
$dir = BASE_DIR;
$real_name = "$dir/Mapics/csv_WebNavData.csv";

$mapics_provider = "http://ulskold.it-russia.org:8083/csv_WebNavData.csv";
if (strlen ($result) == 0) {
  $file = fopen ($mapics_provider, "r");
}
if (!$file) {
  echo "<p>Unable to open remote file. $mapics_provider\n";
  exit;
}
$dir = BASE_DIR;
$out_file = fopen ("$dir/Mapics/csv_WebNavData.csv","w");
if (!$out_file) {
  echo "<p>Unable to create file. $dir/Mapics/csv_WebNavData.csv\n";
  exit;
};
		      
while (!feof ($file)) {
  $line = fgets ($file, 4 * 1024);
